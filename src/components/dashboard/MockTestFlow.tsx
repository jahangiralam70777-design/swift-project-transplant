...
function ExamStage({
  ...
  // Countdown ticker. Uses a ref to the latest mutation so the interval is
  // created once per attempt (questions.length transition) and is not torn
  // down on every render — preventing time-jumps and double-tick artifacts.
  const submitMutRef = useRef(submitMut);
  submitMutRef.current = submitMut;
  useEffect(() => {
    if (!questions.length) return;
    const id = setInterval(() => {
      if (submitMutRef.current.isPending) return;
      setTimeLeft((t) => {
        if (t <= 1) {
          clearInterval(id);
