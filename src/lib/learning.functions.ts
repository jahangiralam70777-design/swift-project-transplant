...
// ---- User-driven quiz generation from MCQ bank ----
const generateUserQuizSchema = z.object({
  subjectId: z.string().uuid(),
  chapterIds: z.array(z.string().uuid()).max(200).optional(),
  count: z.number().int().min(1).max(100).default(20),
  durationMinutes: z.number().int().min(1).max(180).default(20),
});

export const generateUserQuiz = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: z.infer<typeof generateUserQuizSchema>) =>
    generateUserQuizSchema.parse(i),
  )
  .handler(async ({ data, context }) => {
    const sb = context.supabase;
    ...
    // Resolve chapter scope (specific or all published in subject)
    let chapterIds = data.chapterIds ?? [];
    if (chapterIds.length === 0) {
      const { data: chs, error: cErr } = await sb
        .from("chapters")
        .select("id")
        .eq("subject_id", data.subjectId)
        .eq("status", "published");
      if (cErr) throw cErr;
      chapterIds = (chs ?? []).map((c) => c.id as string);
    }
    if (chapterIds.length === 0) {
    ...
    const { data: pool, error: pErr } = await sb
      ...
      .eq("status", "published");
    if (pErr) throw pErr;
    const ids = (pool ?? []).map((m) => m.id as string);
    if (ids.length === 0) {
      throw new Error("No questions are available for the selected chapters yet.");
    }
    for (let i = ids.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [ids[i], ids[j]] = [ids[j], ids[i]];
    }
    const picked = ids.slice(0, Math.min(data.count, ids.length));

    // Title from chapter (single) or subject (multi/all)
    let title = subj.name;
    let singleChapterId: string | null = null;
    if (chapterIds.length === 1) {
      const { data: ch } = await sb
        ...
        .eq("id", chapterIds[0])
        .single();
      title = ch?.name ?? subj.name;
      singleChapterId = ch?.id ?? null;
    }
    ...
    const rows = picked.map((mcqId, idx) => ({
      quiz_id: quiz.id as string,
      mcq_id: mcqId,
      position: idx + 1,
    }));
    const { error: qqErr } = await supabaseAdmin.from("quiz_questions").insert(rows);
    if (qqErr) throw qqErr;
